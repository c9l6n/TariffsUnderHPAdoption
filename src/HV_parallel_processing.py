...
from src import PFA_HV

def process_single_grid(grid_code, random_seeds, HP_percentages):
    """Process a single HV grid configuration"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(script_dir)
    results_dir = os.path.join(base_dir, "data", "results", "HVGridResults")
    os.makedirs(results_dir, exist_ok=True)

    grid_name = grid_code.split('--')[0].replace('1-HV-', '').replace('-', '_')
    filename = f"hv_{grid_name}.csv"
    file_path = os.path.join(results_dir, filename)

    # Run calculation
    results_df = PFA_HV.run_scenarios([grid_code], random_seeds, HP_percentages)
...
def run_parallel_analysis(random_seeds, HP_percentages):
    ...
    try:
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            ...
            print("All HV grids processed successfully")
